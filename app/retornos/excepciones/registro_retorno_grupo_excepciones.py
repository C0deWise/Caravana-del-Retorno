...
class RetornoNoActivo(HTTPException):
    def __init__(self, retorno_id: int, retorno_estado: str, accion: str = "realizar esta acción"):
        super().__init__(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"No es posible {accion} en el retorno con código {retorno_id} porque su estado es {retorno_estado}."
        )
